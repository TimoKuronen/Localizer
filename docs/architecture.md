...
Application is planned as a new project. It coordinates use cases and defines the ports they require:
...
The CLI is the first host and composition root. A future desktop host may reuse the same Application use cases.

Hosts are responsible for configuration, dependency registration, cancellation, progress reporting, and presentation. Business rules do not belong in command handlers or view models.
...
Localizer.Infrastructure
      ^
      |
Localizer.Cli or future desktop host
```
...
Initial profiles should be limited:

- `plain`: no placeholders.
- `composite`: indexed or named replacement fields supported by the first consumer.
...
3. Placeholder structure: required selectors and multiplicity.
4. Formatter structure: formatter names and required options.
5. Markup: balanced allowed tags and attributes.
6. Constraints: configured length, line, or forbidden-term policies.
7. Export policy: required locales are current, approved, and error-free.
...
- Configurable CSV for spreadsheet and tool interchange.
...
- CLI-first vertical slice.
- Explicit human approval.
- Derived Missing and Stale statuses.
- Parser-based, profile-aware message validation.
